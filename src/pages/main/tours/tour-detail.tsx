...
export default function TourDetailPage() {
  ...
  const tour = tours.find((item) => item.id === id && item.category !== "trek")
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center px-6">
        <div className="text-center">
          ...
          <p className="mt-2 text-muted-foreground">The selected tour does not exist.</p>
          <Link to="/tours">
  ...
  const serviceId = Number(tour.id.replace(/\D/g, ""))

  useEffect(() => {
    let mounted = true
    const resolve = async () => {
      const id = await resolveServiceId("tour", tour.name, serviceId)
  ...
  useEffect(() => {
    ...
    const loadCapacity = async () => {
      try {
        const data = await serviceCapacityApi.getById(resolvedServiceId, "tour")
        if (mounted) setCapacity(data)
      } catch {
        // keep booking available if capacity endpoint fails
      }
    }
    void loadCapacity()
  ...
  const handleBook = async () => {
    ...
    try {
      ...
      const booking = await createServiceBooking({
        ...
        serviceLocation: tour.location,
        servicePrice: tour.price
      })
      setStatusMessage("Tour booking created successfully.")
      setResolvedServiceId(Number(booking.service_id))
      const data = await serviceCapacityApi.getById(Number(booking.service_id), "tour")
      setCapacity(data)
    } catch (error) {
          ...
          <div>
            ...
            <div className="mt-8 flex items-center justify-between border-t border-border pt-6">
              ...
              <Button onClick={handleBook} disabled={bookingBusy || isServiceFull}>
                {bookingBusy ? "Booking..." : isServiceFull ? "Sold Out" : "Book Tour"}
              </Button>
            </div>
            {capacity && (
