...
export default function RestaurantDetailPage() {
  ...
  const handleBooked = async (booking: ServiceBooking) => {
    ...
    try {
      const data = await serviceCapacityApi.getById(booking.serviceId, "restaurant")
      setCapacity(data)
    } catch {
    ...
    <div className="min-h-screen bg-background py-10">
          ...
          <div>
            ...
            <div className="mt-8 flex items-center justify-between border-t border-border pt-6">
              ...
              <Button onClick={handleReserve} disabled={isServiceFull}>
                {isServiceFull ? "Sold Out" : "Reserve Table"}
              </Button>
            </div>
            {capacity && (
      ...
      <ServiceBookingFlow
        ...
        service={{
          ...
          price: 0,
          priceSuffix: "reservation",
          scheduleLabel: "Reservation date",
          quantityLabel: "Seats",
          defaultBookingCount: 2,
          maxBookingCount: Math.max(1, capacity?.availableSlots ?? 1),
          minAdvanceDays: 2
        }}
        onBooked={(booking) => {
