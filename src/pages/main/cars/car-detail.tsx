...
export default function CarDetailPage() {
  ...
  const handleReserve = async () => {
    ...
    if (!token || !serviceId) {
      setStatusMessage("Unable to create car reservation right now.")
      return
    }

    setBookingBusy(true)
    try {
      const bookingDate = new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString().slice(0, 19).replace("T", " ")
      const booking = await createServiceBooking({
        ...
        bookingCount: 1,
        status: "pending",
        serviceDescription: "Car rental booking",
        serviceLocation: "",
        servicePrice: car.price
      })
      setStatusMessage("Car rental reserved successfully.")
      setResolvedServiceId(Number(booking.service_id))
      const data = await serviceCapacityApi.getById(Number(booking.service_id), "car")
      setCapacity(data)
    } catch (error) {
      setStatusMessage(error instanceof Error ? error.message : "Car reservation failed")
    } finally {
          ...
          <div>
            ...
            <div className="mt-8 flex items-center justify-between border-t border-border pt-6">
              ...
              <Button onClick={handleReserve} disabled={bookingBusy || isServiceFull}>
                {bookingBusy ? "Reserving..." : isServiceFull ? "Sold Out" : "Rent Now"}
              </Button>
            </div>
            {capacity && (
