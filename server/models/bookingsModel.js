...
const selectClause = `
  ...
  LEFT JOIN services s
   ...
   AND s.service_type = b.service_type
`
...
export const bookingsModel = {
  async getAll() {
    return query(`${selectClause} ORDER BY b.created_at DESC`)
  },

  async getByUserId(userId) {
    return query(`${selectClause} WHERE b.user_id = ? ORDER BY b.created_at DESC`, [userId])
  },

  async getById(id) {
    const rows = await query(`${selectClause} WHERE b.id = ? LIMIT 1`, [id])
    return rows[0] || null
  },

  async getByServiceId(serviceId, serviceType = null) {
    const clauses = ['b.service_id = ?']
    const params = [serviceId]
    if (serviceType) {
      clauses.push('b.service_type = ?')
      params.push(serviceType)
    }
    return query(`${selectClause} WHERE ${clauses.join(' AND ')} ORDER BY b.created_at DESC`, params)
  },
  ...
  async create({
    ...
    return withTransaction(async (connection) => {
      ...
      const [bookingRows] = await connection.execute(`${selectClause} WHERE b.id = ? LIMIT 1`, [bookingResult.insertId])
  ...
  async updateById(id, payload) {
    return withTransaction(async (connection) => {
      const [bookingRows] = await connection.execute(
        `${selectClause} WHERE b.id = ? LIMIT 1 FOR UPDATE`,
        [id]
      )
      ...
      const [updatedRows] = await connection.execute(`${selectClause} WHERE b.id = ? LIMIT 1`, [id])
